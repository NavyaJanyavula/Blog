...
with mysql.connector.connect(host='localhost',user='root',password='system',db='blog'):
    ...
    cursor.execute("create table if not exists registration(username varchar(30)primary key,mobile varchar(10) unique,email varchar(100) unique,address varchar(30),password varchar(20),poster_id int)")
    cursor.execute("create table if not exists posts(id int primary key auto_increment,title varchar(255),content text,date_posted datetime,slug varchar(255),poster_id int,FOREIGN KEY(poster_id)References registration(poster_id))")
